...
struct LFMNuKnob : SvgKnob {
	LFMNuKnob() {
		setSvg(APP->window->loadSvg(asset::plugin(pluginInstance, "res/LFMNuKnob.svg")));
		minAngle = -0.68*M_PI;
		maxAngle = 0.68*M_PI;
	}
};
...
extern Model *modelTimeExpansion;
extern Model *modelTornArtery;
